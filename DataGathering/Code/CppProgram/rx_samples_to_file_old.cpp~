...
void benchmark_rx_rate(
    ...
    const std::string &file, size_t num_of_samps){
    ...
    for (size_t i=0; i<num_chan; i++)
        outfile[i].open((boost::format("%s") % file ).str().c_str(), std::ofstream::binary);
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    std::string args, file, ant, subdev, ref, wirefmt, cpufmt, channel_list;
