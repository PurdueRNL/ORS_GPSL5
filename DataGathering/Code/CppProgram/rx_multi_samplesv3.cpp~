...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    std::cout << boost::format("Channels    %d") % (usrp->get_rx_num_channels())<< std::endl;
	...
	while (f < 1) {
		...
		if ((acc_rx_samps>rate) || (f==0)){
            ...
            std::cout << boost::format("Results:") << std::endl;
            std::cout << boost::format("Rate: %d") % rate << std::endl;
            std::cout << boost::format("Samples accepted: %d") % acc_rx_samps << std::endl;
            std::cout << boost::format("Overflows: %d") % num_overflows << std::endl;
            std::cout << boost::format("Drops: %d") % num_dropped_samps << std::endl;
			...
			for (size_t i = 0; i < 1; i++) {
                    ...
                    // Write signal environment information to file
