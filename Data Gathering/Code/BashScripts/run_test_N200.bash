...
dtype="sc8" # Data type
runs_this=60 # Number of datafiles to collect before reconnecting to N200
...
while (($run <= ($nfiles / $runs_this))); do
    ...
    ../CppProgram/rx_samples_to_file --args "$addr" --time $rtime --runs_this $runs_this --nsamp $nsamp --rate $rate --subdev "$subdev" --freq $freq --channels "0" --rfile "$rfile" --dfile "$dfile" --gain "$gain" --wirefmt "$dtype" --cpufmt "$dtype"
